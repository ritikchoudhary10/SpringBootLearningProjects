package com.ritik;

import java.util.List;

import org.springframework.boot.SpringApplication;
import org.springframework.boot.autoconfigure.SpringBootApplication;
import org.springframework.context.ApplicationContext;

import com.ritik.bean.Employee;
import com.ritik.controller.EmployeeManagerController;

@SpringBootApplication
public class SbProj12ProfilesusingPropertiesApplication {

	public static void main(String[] args) {
		ApplicationContext ctx = SpringApplication.run(SbProj12ProfilesusingPropertiesApplication.class, args);
		EmployeeManagerController bean = ctx.getBean("empController",EmployeeManagerController.class);
		try {
			List<Employee> list=bean.getDetails("MANAGEr", "ANALYST", "SALESMAN");
			if(!list.isEmpty()) {
				
			list.forEach((emp)->{
				System.out.println(emp);
			});
			}
			else {System.out.println("Designation not entered properly check spelling");}
		}catch(Exception e) {
			e.printStackTrace();
		}
	}

}
